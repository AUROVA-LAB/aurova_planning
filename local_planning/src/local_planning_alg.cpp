...
void LocalPlanningAlgorithm::potentialForcesMap(pcl::PointCloud<pcl::PointXYZ> free_space, cv::Point2f goal_lidar,
                                                PFConfig &pf_config, vector<vector<cv::Point> > &contour,
                                                cv::Mat &pf_map)
{
  float in_out_flag;
...
void LocalPlanningAlgorithm::findTransitableAreas(cv::Mat pf_map, vector<vector<cv::Point> > contour,
                                                  cv::Point2f goal_lidar, PFConfig pf_config, cv::Mat &roads_map)
{
...
void LocalPlanningAlgorithm::findLocalGoal(pcl::PointCloud<pcl::PointXYZ> free_space, cv::Point2f goal_lidar,
                                           PFConfig &pf_config, vector<vector<cv::Point> > &contour, int &radious,
                                           cv::Mat &plot_img, vector<cv::Point2d> &goal_candidates,
...
void LocalPlanningAlgorithm::findControlAction(pcl::PointCloud<pcl::PointXYZ> free_space, cv::Point2d local_goal,
                                               Pose2D base_pose, cv::Point2f goal_lidar, PFConfig pf_config,
                                               CtrlConfig ctrl_config, vector<vector<cv::Point> > contour,
                                               ...
                                               cv::Mat &plot_img)
{
  ...
  for (float st = -1 * ctrl_config.max_angle; st < ctrl_config.max_angle; st += ctrl_config.delta_angle)
  {
    ...
    if (in_out_flag > 0)
    {
      ...
      {
        distance_r = sqrt(
            pow((float)(free_space.points[k].x - pose_x), 2.0) + pow((float)(free_space.points[k].y - pose_y), 2.0));
        distance_f = sqrt(
            pow((float)(free_space.points[k].x - pose_front_x), 2.0)
  ...
  if (min_sp < 0.0)
    ackermann_state.drive.speed = -1 * ackermann_state.drive.speed;
